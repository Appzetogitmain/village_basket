...
export default function DateTrackerStrip({ hideLegend = false }: { hideLegend?: boolean }) {
  ...
  return (
    <div className={`w-full px-4 ${hideLegend ? 'mb-4' : 'mb-6'} relative z-20`}>
        ...
        {dates.map((date, idx) => {
          ...
          return (
            <button
              ...
              className={`flex-shrink-0 w-12 h-12 rounded-full flex flex-col items-center justify-center transition-all duration-300 active:scale-95 relative ${
                isSelected 
                  ? 'bg-[#8B3D28] text-white shadow-md scale-105 z-10' 
                  : 'bg-white border border-neutral-100 text-village-umber shadow-sm hover:border-[#8B3D28]/30'
              }`}
            >
              <span className={`text-[7px] font-bold uppercase leading-none ${isSelected ? 'text-white/80' : 'text-neutral-400'}`}>
                {format(date, 'EEE')}
              </span>
              ...
              <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 flex gap-0.5">
                {dotColor && <div className={`w-1.5 h-1.5 rounded-full ${dotColor} border border-white shadow-sm`}></div>}
                {isToday && !dotColor && <div className={`w-1 h-1 rounded-full ${isSelected ? 'bg-white/50' : 'bg-[#8B3D28]/30'}`}></div>}
              </div>
            </button>
